...
fields_cell = cat(2,default_fields,invar);
for field_num = 1:size(fields_cell,2)
    fields.(fields_cell{field_num}) = [];
end
...
if ~isempty(strfind(d(3).name,'.dcm'))
    ...
    datasets = printScansGE_pfile(directory,d,exp_name);    
end

end

function datasets = printScansGE_pfile(directory,d,exp_name)
    pfile_only_index = 1;
    for d_field_num = 1:size(d,1)
        ...
        i = strfind(d(d_field_num).name,'.7_');
