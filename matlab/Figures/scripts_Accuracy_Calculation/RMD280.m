% load images
reconstruct('RMD280',[6 14]);
time_axis_hot=linspace(0+1,67-1,30);
temperature_measurements_hot=RMD280_Sc14_HOT.t_ave_uncorr(:,1,78,58);
...
rmsd_RMD280=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot).^2));
...
rmsd_rel_RMD280=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot_corr).^2));
...
imagesc(squeeze(abs(RMD280_Sc6_rare(2,:,:))));
axis image off
colormap gray
