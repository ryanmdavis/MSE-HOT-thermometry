% load images
reconstruct('RMD278',[3 12]);
time_axis_hot=linspace(0+1,67-1,30);
% temperature_measurements_hot=RMD278_Sc12_HOT.t_ave(:,1,1,81,67);
temperature_measurements_hot=RMD278_Sc12_HOT.t_ave_uncorr(:,1,79,58);

% load Luxtron data
load(strcat(hotPath('luxtron'),'\RMD278 luxtron data'));
...
rmsd_RMD278=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot).^2));
...
rmsd_rel_RMD278=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot_corr).^2));
...
imagesc(squeeze(abs(RMD278_Sc3_rare(1,:,:))));
axis image off
colormap gray
