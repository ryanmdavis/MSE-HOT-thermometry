...
reconstruct('RMD281',[5 12]);
time_axis_hot=linspace(0+1,67-1,30);
temperature_measurements_hot=RMD281_Sc12_HOT.t_ave_uncorr(:,1,72,63);

% load Luxtron data
load(strcat(hotPath('luxtron'),'\RMD281 luxtron data'));
...
rmsd_RMD281=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot).^2));
...
rmsd_rel_RMD281=sqrt(mean((temperature_measurements_lux(lux_bins_compare)-temperature_measurements_hot_corr).^2));
